							...
							<div class="card-body">
								<div class="basic-form">
									<form action = "{{ route($sroute_prefix.'edit_simpan', ['id' => $id ] ) }}" method="POST">
										...
										<div class="form-group row">
											<label class="col-sm-2 col-form-label">Tahun Aset</label>
											<div class="col-sm-10">
												<input type="text" class="form-control" name = "tahun_aset" value = "{{ $data->tahun_aset }}">
											</div>
										</div>
										<div class="form-group row">
											<label class="col-sm-2 col-form-label">Ruang</label>
											<div class="col-sm-10">
												<select class="form-control" id="sel1" name = "idruang">
													<option value = "0"> - </option>
													@foreach($ruang as $row)
														<option value = "{{ $row->idruang }}" @if($data->idruang == $row->idruang) selected @endif>{{ $row->nama_ruang }}</option>
													@endforeach
												</select>
											</div>
										</div>
