								...
								<div class="basic-form">
									<form action = "{{ route($sroute_prefix.'edit_simpan', ['id' => $id ] ) }}" method="POST">
										...
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<input type="text" class="form-control" name = "nama_program_studi" value = "{{ $data->nama_program_studi }}">
											</div>
										</div>
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<select class="form-control" id="sel2" name = "idfakultas">
													...
													@foreach($fakultas as $row)
														<option value = "{{ $row->idfakultas }}" @if($data->idfakultas == $row->idfakultas) selected @endif>{{ $row->nama_fakultas }}</option>
													@endforeach
												</select>
											</div>
										</div>
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<select class="form-control" id="sel2" name = "idjenjang">
													<option value = "0"> - </option>
													@foreach($jenjang as $row)
														<option value = "{{ $row->idjenjang }}" @if($data->idjenjang == $row->idjenjang) selected @endif>{{ $row->nama_jenjang }}</option>
													@endforeach
												</select>
											</div>
										</div>
