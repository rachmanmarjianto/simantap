						...
						<div class="card">
							...
							<div class="card-body">
								<div class="basic-form">
									<form action = "{{ route('layanan_edit_simpan', ['id' => $id ] ) }}" method="POST">
										@csrf
											...
											<div class="col-sm-10">
												<select class="form-control" id="sel1" name = "idunit_kerja">
													@foreach($unit_kerja as $row)
														<option value = "{{ $row->idunit_kerja }}" @if($layanan->idunit_kerja == $row->idunit_kerja) selected @endif>{{ $row->nama_unit_kerja }}</option>
													@endforeach
												</select>
											</div>
										</div>
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<select class="form-control" id="sel2" name = "status">
													<option value = "0" @if($layanan->status == '0') selected @endif>Tidak Aktif</option>
										...
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<input type="text" class="form-control" name = "idlayanan_unit_kerja" value = "{{ $layanan->idlayanan_unit_kerja }}">
											</div>
										</div>
										<div class="form-group row">
											<div class="col-sm-10">
