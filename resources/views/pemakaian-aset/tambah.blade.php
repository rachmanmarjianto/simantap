								...
								<div class="basic-form">
									<form action = "{{ route($sroute_prefix.'simpan') }}" method="POST">
										...
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<input type="text" class="form-control" name = "timestamp_mulai" value = "{{ date('Y-m-d H:i:s') }}">
											</div>
										</div>
										<div class="form-group row">
										...
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<input type="text" class="form-control" name = "timestamp_akhir" value = "{{ date('Y-m-d H:i:s') }}">
											</div>
										</div>
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<select class="form-control" id="sel1" name = "idpermintaan_layanan">
													...
													@foreach($permintaan_layanan as $row)
														<option value = "{{ $row->idpermintaan_layanan }}">{{ $row->layanan->nama_layanan }}</option>
													@endforeach
												</select>
											</div>
										</div>
