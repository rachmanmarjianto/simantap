								...
								<div class="basic-form">
									<form action = "{{ route($sroute_prefix.'edit_simpan', ['id' => $id ] ) }}" method="POST">
											...
											<div class="col-sm-10">
												<select class="form-control" id="sel1" name = "idlayanan">
													...
													@foreach($layanan as $row)
														<option value = "{{ $row->idlayanan }}" @if($data->idlayanan == $row->idlayanan) selected @endif>{{ $row->nama_layanan }}</option>
													@endforeach
												</select>
											</div>
										</div>
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<input type="text" class="form-control" name = "waktu_penggunaan_ideal_min" placeholder="Dalam menit" value="{{ $data->waktu_penggunaan_ideal_min }}">
											</div>
										</div>
										<div class="form-group row">
											<div class="col-sm-10">
